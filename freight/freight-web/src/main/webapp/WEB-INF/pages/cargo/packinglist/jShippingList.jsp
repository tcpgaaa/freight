...
<head>
    ...
    <script>
        ...
        function toView() {
            if (isOnlyChecked()) {
                formSubmit('${ctx}/shippingOrder/toView', '_self');
            } else {
        ...
        function toUpdate() {
            if (isOnlyChecked()) {
                formSubmit('${ctx}/shippingOrder/toUpdate', '_self');
            } else {
                alert("请先选择一项并且只能选择一项，再进行操作！");
            }
        }
        ...
        function toCal() {
            if (isOnlyChecked()) {
                formSubmit('${ctx}/shippingOrder/cancel', '_self');
            } else {
                alert("请先选择一项并且只能选择一项，再进行操作！");
            }
        }
        ...
        function toSub() {
            if (isOnlyChecked()) {
                formSubmit('${ctx}/shippingOrder/submit', '_self');
            } else {
                alert("请先选择一项并且只能选择一项，再进行操作！");
            }
        }
        ...
        function toShip() {
            if (isOnlyChecked()) {
                formSubmit('${ctx}/shippingOrder/entrust', '_self');
            } else {
                alert("请先选择一项并且只能选择一项，再进行操作！");
            }
        }
            ...
            <table id="ec_table" class="tableRegion" width="98%">
                ...
                <c:forEach items="${pb.datas}" var="o" varStatus="status">
                    <tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'">
                        ...
                        <td>
                            <c:if test="${o.shippingOrderState==0}">草稿</c:if>
                            <c:if test="${o.shippingOrderState==1}"><b><font color="green">已上报</font></b></c:if>
                        </td>
                    </tr>
                </c:forEach>
